...
namespace ProdStore
{
    public class OrderItemCollection : IReadOnlyCollection<OrderItem>
    {
        ...
        public OrderItem Add(int productId, decimal productPrice,int count)
        {
            if (TryGet(productId, out OrderItem orderItem))
                throw new InvalidOperationException("Prodcut Already exist");
        ...
        public void Remove(int prodcutId)
        {
            var index = items.FindIndex(item => item.ProductId == prodcutId);
            if (index == -1)
                throw new InvalidOperationException("Can't find product to remove");
